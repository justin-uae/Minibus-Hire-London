            ...
            <div className="relative bg-gradient-to-r from-blue-600 to-blue-700 text-white py-20 md:py-28 overflow-hidden">
                ...
                <div className="relative max-w-7xl mx-auto px-4 sm:px-6 text-center z-10">
                    <h1 className="text-4xl sm:text-5xl md:text-6xl font-black mb-6 animate-fade-in">
                        Minibus Hire <span className="text-blue-200">Blog</span>
                    </h1>
                    <p className="text-xl md:text-2xl text-blue-100 mb-8 animate-fade-in-delay max-w-3xl mx-auto">
                    ...
                    <div className="max-w-2xl mx-auto w-full animate-fade-in-delay-2">
                        <div className="relative group">
                            <Search className="absolute left-4 sm:left-5 top-1/2 -translate-y-1/2 w-5 h-5 md:w-6 md:h-6 text-gray-400 group-focus-within:text-blue-600 transition-colors z-10" />
                            <input
                                type="text"
                                placeholder="Search travel tips, guides, destinations..."
                                value={searchQuery}
                                onChange={(e) => setSearchQuery(e.target.value)}
                                className="relative w-full pl-12 sm:pl-14 md:pl-16 pr-12 sm:pr-14 py-4 sm:py-5 md:py-6 rounded-xl sm:rounded-2xl bg-white text-gray-900 text-sm sm:text-base md:text-lg focus:outline-none focus:ring-4 focus:ring-blue-300 shadow-xl font-medium border-2 border-transparent focus:border-blue-400 transition-all"
                            />
                            {searchQuery && (
                                <button
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 md:py-12">
                ...
                <div className="mb-6">
                    <p className="text-gray-600 font-medium">
                        Showing <span className="text-blue-600 font-bold text-lg">{filteredBlogs.length}</span>
...
const BlogCard = ({ blog, onClick }: { blog: BlogPost; onClick: () => void }) => (
    ...
    >
        <div className="relative overflow-hidden" style={{ minHeight: '200px', maxHeight: '240px' }}>
            ...
            <div className="absolute top-3 left-3 bg-white/95 backdrop-blur-sm px-3 py-1.5 rounded-full text-xs font-bold text-blue-600 border border-blue-200 shadow-lg">
                {blog.category}
            </div>
        </div>

        <div className="p-5">
            <div className="flex items-center gap-3 mb-2 text-xs text-blue-600">
            ...
            <div className="flex items-center justify-between">
                ...
                <span className="text-xs font-bold text-blue-600">Read More →</span>
            </div>
        </div>
    </div>
);
...
const EmptyState = ({ searchQuery, onClear }: { searchQuery: string; onClear: () => void }) => (
    <div className="text-center py-20 bg-white rounded-2xl shadow-lg border-2 border-gray-200">
        ...
        <button
            onClick={onClear}
            className="px-8 py-4 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-xl font-bold shadow-lg hover:shadow-xl transition-all hover:scale-105"
        >
            Clear Filters
        </button>
    </div>
